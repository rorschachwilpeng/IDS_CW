...
filename = 'KNN Trained Model.sav'
...
result_test.to_excel('../OUTPUT/KNN_confusion_matrix.xls')
...
fpr_test_KNN,tpr_test_KNN,threshold_test_KNN = roc_curve(y_test_2018,predictions_test[:,1])
roc_auc_test_KNN = auc(fpr_test_KNN,tpr_test_KNN)
print('The Testing Set AUC: ',roc_auc_test_KNN)
# fpr,tpr,threshold = roc_curve(y_test,predictions)
# roc_auc = auc(y_test, predictions)
plt.plot(fpr_test_KNN, tpr_test_KNN, label='ROC curve (area = %0.3f)' % roc_auc_test_KNN)
plt.plot([0, 1], [0, 1], 'k--')  # random predictions curve
plt.plot([0, 0.5], [1, 0.5], 'k--')  # random predictions curve
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.0])
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Receiver Operating Characteristic of KNN(Test Set)')
plt.legend(loc="lower right")
plt.savefig('../OUTPUT/KNN_ROC-AUC Curve(Test).png')
plt.show()

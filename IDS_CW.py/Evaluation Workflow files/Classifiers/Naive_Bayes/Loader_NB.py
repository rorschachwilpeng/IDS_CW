...
result_test.to_excel('../OUTPUT/NB_confusion_matrix.xls')
...
# test DATASET ROC-AUC DIAGRAM
# predictions_test = model_balanced.predict_proba(x_test_combined)
# fpr_test,tpr_test,threshold_test = roc_curve(y_test_combined,predictions_test[:,1])
# roc_auc_test = auc(fpr_test,tpr_test)
# print('The Traning Set AUC: ',roc_auc_test)
...
# plt.savefig('../OUTPUT//ROC-AUC Curve(test).png')
# plt.show()

# Testing DATASET ROC-AUC DIAGRAM
predictions_test = model_balanced.predict_proba(x_test_2018)
fpr_test_NB,tor_test_NB,threshold_test_NB = roc_curve(y_test_2018,predictions_test[:,1])
roc_auc_test_NB = auc(fpr_test_NB,tor_test_NB)
print('The Testing Set AUC: ',roc_auc_test_NB)
# fpr,tpr,threshold = roc_curve(y_test,predictions)
# roc_auc = auc(y_test, predictions)
plt.plot(fpr_test_NB, tor_test_NB, label='ROC curve (area = %0.3f)' % roc_auc_test_NB)
plt.plot([0, 1], [0, 1], 'k--')  # random predictions curve
plt.plot([0, 0.5], [1, 0.5], 'k--')  # random predictions curve
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.0])
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Receiver Operating Characteristic of Naive Bayes(Test Set)')
plt.legend(loc="lower right")
plt.savefig('../OUTPUT/NB_ROC-AUC Curve(Test).png')
plt.show()
